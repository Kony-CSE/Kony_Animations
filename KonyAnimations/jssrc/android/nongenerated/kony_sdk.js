...
function konyNetHttpRequestCallback(result) {
    ...
    if (readyState === 4) {
        ...
        if (response && !(isInvalidJSON)) {
            ...
            response.httpresponse.url = httpRequest.url;
        ...
        if (isInvalidJSON || (!response && status == 200)) {
            ...
            errorMessage.httpresponse.url = httpRequest.url;
            errorMessage.httpresponse.responsecode = status;
            httpRequest.failureCallback(errorMessage);
        } else if (status === 200) {
            if (!response.opstatus) {
                response.opstatus = 0;
            }
            if (response.opstatus == 0) {
                httpRequest.successCallback(response);
            } else {
                httpRequest.failureCallback(response);
            }
        } else {
            var resultTable = {};
            if (response) {
            ...
            httpRequest.failureCallback(resultTable);
        }
    }
}
...
function konyNetHttpRequest(url, params, headers, successCallback, failureCallback) {
    ...
    httpRequest.url = url;
    httpRequest.successCallback = successCallback;
    httpRequest.failureCallback = failureCallback;
    var firstKey = true;
    ...
    httpRequest.onReadyStateChange = konyNetHttpRequestCallback;
